...
from ..base import base_list, table_to_string_mapping, make_namespace, TaskNotifier, extract_ids
...
socketio.on_event('list', base_list(Sample, sample_schema, SAMPLE_NAMESPACE), namespace=make_namespace(SAMPLE_NAMESPACE))
...
def get_sample(json):
    ...
    for sample_id in ids:
        ...
        if sample:
            ...
            wells += [_.well for _ in channels]
    ...
    sample_dump = sample_schema.dumps([sample], many=True)
    socketio.emit('get', {SAMPLE_NAMESPACE: sample_dump.data}, namespace=make_namespace(SAMPLE_NAMESPACE))
    socketio.sleep()


@microspat.route('/sample/upload_samples/', methods=['POST', 'OPTIONS'])
def upload_samples():
