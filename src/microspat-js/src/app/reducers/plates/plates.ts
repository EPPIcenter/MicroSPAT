...
import * as PlatesActions from '../../actions/plates';
...
import { Trace, Legend } from '../../containers/components/plots/canvas';
import { Locus } from '../../models/locus/locus';
import { Task, IN_PROGRESS } from '../../models/task';
...
function dbDeleteReceived(state: State, action: DBActions.DeleteReceivedAction): State {
  ...
  if (action.payload.model === fromDB.models.plate &&
      deletedIds.indexOf(+state.activePlateId) >= 0) {
    return Object.assign({}, state, {
      loadingPlate: null,
      activePlateId: null,
      loadingWell: null,
...
function activatePlate(state: State, action: PlatesActions.ActivatePlateAction): State {
  if (action.payload && !(+action.payload === +state.activePlateId)) {
    return Object.assign({}, state, {
      loadingPlate: null,
