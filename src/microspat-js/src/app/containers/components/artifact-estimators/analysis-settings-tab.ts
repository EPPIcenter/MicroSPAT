...
@Component({
  selector: 'mspat-artifact-estimator-analysis-settings',
      ...
      <div class="col-sm-5 settings-card mat-elevation-z8">
        <form [formGroup]="form">
          <div class="row">
            ...
            <mspat-artifact-estimator-parameters [parameters]="selectedLocusParameters" class="col-sm-6"></mspat-artifact-estimator-parameters>
          </div>
          <div class="row">
...
export class ArtifactEstimatorAnalysisSettingsComponent implements OnChanges {
  ...
  @Input() defaultParameters = {
    ...
    // Artifact Estimator Parameters
    max_secondary_relative_peak_height: .4,
    min_artifact_peak_frequency: 10
  }

  @Output() analyzeLoci = new EventEmitter();

  form: FormGroup = new FormGroup({});
  locusParameterSelection: SelectionModel<ArtifactEstimatorLocusParams>;

  constructor() {
    const initiallocusParameterSelection = [];
    const allowMultiSelect = true;
    this.locusParameterSelection = new SelectionModel<ArtifactEstimatorLocusParams>(allowMultiSelect, initiallocusParameterSelection);
  }
  ...
  ngOnChanges(c: SimpleChanges) {
    if (c.activeLocusParameters) {
      const initialLocusParameterSelection = [];
      const allowMultiSelect = true;
      this.locusParameterSelection = new SelectionModel<ArtifactEstimatorLocusParams>(
  ...
  prepareParameterSettings() {
    return Object.assign({},
      this.form.get('artifact_estimator_parameters').value,
