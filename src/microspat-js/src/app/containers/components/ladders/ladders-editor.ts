...
export class LadderEditorComponent implements OnChanges {
  ...
  baseSizeValidator(ctrl: AbstractControl): {[key: string]: any} | null {
    const baseSizes = ctrl.value.split(',').map((t: string | number) => +t).every(Number.isInteger);
    return baseSizes ? null : {'improperBaseSizes': ctrl.value};
  }

  ngOnChanges(_change: any) {
    if (this.ladder) {
  ...
  prepareSaveLadder(): Ladder {
    ...
    const newLadder: Ladder = {
      ...
      base_sizes: ladderModel.baseSizeString.split(',').map((t: string | number) => +t),
      ...
      index_overlap: ladderModel.indexOverlap,
      min_time: ladderModel.minTime,
      max_peak_height: ladderModel.maxPeakHeight,
      min_peak_height: ladderModel.minPeakHeight,
      ...
      maxima_window: ladderModel.maximaWindow,
      ...
      cwt_min_width: ladderModel.cwtMinWidth,
      cwt_max_width: ladderModel.cwtMaxWidth,
